...
function fecharCaixa(){
  ...
  const vendasDia=db.vendas.filter(v=>v.data.startsWith(hoje));
  const despesasDia=db.despesas.filter(d=>d.data===hoje);

  const totalVendas=vendasDia.reduce((s,v)=>s+v.total,0);
  const totalDespesas=despesasDia.reduce((s,d)=>s+d.valor,0);
  const lucro=totalVendas-totalDespesas;

  resumoCaixa.innerHTML=`Vendas: R$ ${totalVendas.toFixed(2)}<br>Despesas: R$ ${totalDespesas.toFixed(2)}<br><b>Lucro: R$ ${lucro.toFixed(2)}</b>`;

  const {jsPDF}=window.jspdf;
  const pdf=new jsPDF();
  pdf.text("FECHAMENTO DE CAIXA",10,10);
  pdf.text("Data: "+hoje,10,20);
  pdf.text("Vendas: R$ "+totalVendas.toFixed(2),10,30);
  pdf.text("Despesas: R$ "+totalDespesas.toFixed(2),10,40);
  pdf.text("Lucro: R$ "+lucro.toFixed(2),10,50);
  pdf.save("fechamento-caixa-"+hoje+".pdf");
}
